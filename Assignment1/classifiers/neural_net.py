...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        hidden_layer = np.maximum(0, np.dot(X,W1)+b1) #Using ReLU
        ...
        #Softmax Classifier Loss - removing numerical unstability
        exp_scores = np.exp(scores - np.max(scores, axis=1, keepdims=True))
        probs = exp_scores/np.sum(exp_scores,axis=1,keepdims=True)
        logprobs = -np.log(probs[np.arange(num_train),y])
        
        #Total Loss
        loss = np.sum(logprobs)/num_train+0.5*reg*np.sum(W1*W1)+0.5*reg*np.sum(W2*W2)
        ...
        grads['W2'] = np.dot(hidden_layer.T,d_scores)
        grads['b2'] = np.sum(d_scores,axis=0,keepdims=True)

        # Backpropagation 1
        d_hidden = np.dot(d_scores,W2.T)

        # No Gradient when ReLU was 0
        d_hidden[hidden_layer==0] = 0

        # Backpropagation 2
        grads['W1'] = np.dot(X.T,d_hidden)
        grads['b1'] = np.sum(d_hidden, axis=0, keepdims=True)
